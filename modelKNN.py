...
from keras.layers import Conv2D, MaxPooling2D, MaxPooling3D, Conv3D
...
for objet, val1 in data_objets_leves.items():
	for pixel in json.loads(val1):
		pixels_leves.append(pixel)

pixels_non_leves = []
for objet, val1 in data_objets_non_leves.items():
	for pixel in json.loads(val1):
		pixels_non_leves.append(pixel)
...
print(x_train.shape)
model = Sequential()
model.add(Dense(1000, activation='relu',input_shape=mon_shape))
