	...
	for pixel in json.loads(pixels):
		...
		for numsignal in range(0,10):
			...
			if datePrev != None and datePrevPrev != None and datePrevPrevPrev !=None:
				...
				signal_prev_prev_prev = data[datePrevPrevPrev][numsignal][pixel[0]][pixel[1]]
				signaux_prev_prev_prev.append(signal_prev_prev_prev)
		matrice.append(signaux)
		matrice_prev.append(signaux_prev)
		matrice_prev_prev.append(signaux_prev_prev)
		matrice_prev_prev_prev.append(signaux_prev_prev_prev)
	if len(signaux_prev)>0 and len(signaux_prev_prev)>0 and len(signaux_prev_prev_prev)>0:
		m = []
		m.append(matrice)
		m.append(matrice_prev)
		m.append(matrice_prev_prev)	
		m.append(matrice_prev_prev_prev)	
...
file = open("../data_4_tp/objets_splites_leves_quatre_dates.json","w")
json.dump(objets_levees_dump,file, indent=True)
file.close()
file = open("../data_4_tp/objets_splites_non_leves_quatre_dates.json","w")
